...
export default function TransactionEditTitleScreen() {
  const router = useRouter();
  const { data } = useLocalSearchParams<{ data?: string }>();
  const foreground = useCSSVariable("--color-foreground");
  const muted = useCSSVariable("--color-muted-foreground");
  const tx = useMemo(() => parseTxParam(data), [data]);
  const [value, setValue] = useState(tx?.title ?? "");
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const nextTx: TransactionPayload = { ...tx, title: value };
  ...
  return (
    ...
    >
      <View className="flex-row items-center justify-between">
        <Text className="text-base font-semibold text-foreground">Edit title</Text>
        <Pressable
          onPress={() => router.back()}
